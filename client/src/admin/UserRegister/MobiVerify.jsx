...
const MobiVerify = () => {
    ...
    const handleSubmit = async(e) => {
        ...
        const response=await axios.post(`http://localhost:8000/api/addUser/cors-verifyOtp`,userEnteredOtp)
        console.log(response.data)

        if (response.data.success
            ==='true') {
            navigate('/register', { state: { region, mobile_no } });
        } else {
    ...
    const resendOtp = () => {
        ...
        // Logic to resend OTP  API=http://localhost:8000/api/addUser/cors-verifyOtp
    };
