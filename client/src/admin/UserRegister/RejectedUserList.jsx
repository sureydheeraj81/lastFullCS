...
const RejectedUserList = () => {
  ...
  const API_URL = 'http://localhost:8000/api/addUser/rejectedData'; 
        ...
        <tbody>
          {applications.map((app, index) => (
            <tr key={app.application_no}>
              ...
              <td><div>{app.is_rejected}</div><div>{app.rejected_reason
              }</div></td>
              <td>{app.updatedBy}</td>
              <td>
                <button className="btn btn-outline-primary btn-sm">
                  <Link to={`/approved/${app.sno}`} className="text-decoration-none text-dark">
